...
core.getUser = function(cb) {
  var user = this.getUserSync();
  if (!user) return cb('User not found! Please login first.');

  return cb(null, user);
};

core.getUserSync = function() { return cache.get('.user'); };
core.isLogin = function() { return this.getUserSync() != null; };
